            ...
            <div className="navbar-end hidden lg:flex">
                <ul className="menu menu-horizontal p-0">
                    ...
                    <li tabIndex={0}>
                        ...
                        <ul className="p-2 bg-base-100">
                            <li className="dropdown">
                                <Link to="/ContinentDetail/Europe" className="bg-gray-200 hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap">Europe</Link>
                                {destinationEntries.map(e => e.travelRegion == "Europe" && <li className="bg-gray-200 hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap"><Link to={`/DestinationDetail/${e.title}`}>{e.title}</Link></li>)}
                                <ul className="dropdown-content absolute hidden text-gray-700 pl-5 ml-24 -mt-10">
                                    {destinationEntries.map(e => e.travelRegion == "Europe" && <li className="bg-gray-200 hover:bg-gray-400 py-2 px-4 block whitespace-no-wrap"><Link to={`/DestinationDetail/${e.title}`}>{e.title}</Link></li>)}
                                </ul>
                            </li>
